...
public class SimplePin {
	...
	private SimplePinState currentState = SimplePinState.OFF;
	 ...
	 * @param _name
	 */
	public SimplePin(Pin _pin, String _name) {
		pin = _pin;
		name = _name;
	 ...
	 * @param _name
	 */
	public SimplePin(Pin _pin, String _name, boolean neverAllowFlashing) {
		pin = _pin;
		name = _name;
		thisLED = gpio.provisionDigitalOutputPin(pin, name, PinState.LOW);
	 ...
	 */
	public void setState(SimplePinState state) {
		currentState = state;
		update();
		if (state == SimplePinState.ON) {
			on = true;
		} else if (state == SimplePinState.OFF) {
			on = false;
		}
	}
	
	public SimplePinState getState() {
		return currentState;
	}
	 ...
	 */
	public void update() {
		if (currentState.equals(SimplePinState.OFF)) {
			thisLED.low();
		} else if (currentState.equals(SimplePinState.ON)) {
			thisLED.high();
		}
	}
	/**
	...
	public void startTimer() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (runTimer) {
					if (currentState.equals(SimplePinState.FLASHING_FAST) && !NO_FLASHING_EVER) {
						if (on) {
							thisLED.low();
							on = false;
						} else {
					...
					} else if (currentState.equals(SimplePinState.FLASHING_SLOW) && !NO_FLASHING_EVER) {
						if (counter == 1) {
							if (on) {
								thisLED.low();
								on = false;
					...
					synchronized (this) {
						try {
							this.wait(Reference.PIN_TIMER_INTERVAL);
